    ...
    const results = await Promise.all(
      schools.map(async (school) => {
        try {
          ...
          return { schoolName: school.name, link, status: "success" };
        } catch (err) {
          console.error(`Error generating report for ${school.name}:`, err.message);
          return { schoolName: school.name, error: err.message, status: "failed" };
        }
      })
    );
...
//generate report for one school by name from the aprameter
app.get('/generate-report/:schoolName', async (req, res) => {
    const { schoolName } = req.params;
    try {
        const school = await School.findOne({ name: schoolName });
        if (!school) {
            return res.status(404).send(`School not found: ${schoolName}`);
        }
        const link = await generateAssignmentReportForSchool(school);
        res.send(`Report for ${schoolName}: ${link}`);
    } catch (error) {
        res.status(500).send(`Error generating report for ${schoolName}: ${error.message}`);
    }
});
...
app.get('/reports/:schoolName', async (req, res) => {
  ...
  app.get("/generateforall", async (req, res) =>{
    try{
